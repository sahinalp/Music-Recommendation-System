...
class SongRepository {
  ...
  Future<SongModel> getRandomSong(int random) async {
    SongModel song;
    final event = await connection.query('select * from "MusicRecommendation"."database" d where id =$random order by popularity desc \n');
    try {
  ...
  Future<List<SongModel>> search(String query) async {
    final songs = <SongModel>[];
    await connection.query("select * from \"MusicRecommendation\".\"database\" d where songname like '$query%' LIMIT 20\n").then((event) {
      final songsList = event;
      for (var songMap in songsList) {
        final song = SongModel.fromJson(songMap);
        songs.add(song);
